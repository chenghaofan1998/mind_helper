...
> 范围限定：P1 截图链路只做「选区 → 预览/重选 → 分析 → 确认问题 → 查询」。每个状态只有一个主按钮；遮挡编辑、region 级引用、OCR 分区展示属 P2，见 `IMPLEMENTATION-NOTES.md` 05/06。
...
5. P1 最终展示截图观察与知识库原文，且原文处于视觉主位；Derived 仅保留契约字段，是否渲染留待 P2 验证。
